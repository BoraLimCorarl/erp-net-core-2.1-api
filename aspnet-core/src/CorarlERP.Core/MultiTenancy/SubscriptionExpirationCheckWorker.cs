        ...
        protected override void DoWork()
        {
            ...
            foreach (var tenant in subscriptionExpiredTenants)
            {
                ...
                try
                {
                    ...
                    {
                        _userEmailer.TryToSendSubscriptionExpireEmail(this.AppName, tenant.Id, utcNow);
                    }
                    else if (endSubscriptionResult == EndSubscriptionResult.AssignedToAnotherEdition)
                    {
                        AsyncHelper.RunSync(() => _userEmailer.TryToSendSubscriptionAssignedToAnotherEmail(this.AppName, tenant.Id, utcNow, edition.ExpiringEditionId.Value));
                    }
                }
