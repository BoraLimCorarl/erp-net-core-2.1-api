...
using CorarlERP.Web.Configuration;
...
namespace CorarlERP.Web
{
    ...
    public class CorarlERPWebCoreModule : AbpModule
    {
        private readonly IHostingEnvironment _env;
        private readonly IConfigurationRoot _appConfiguration;

        public CorarlERPWebCoreModule(IHostingEnvironment env)
        {
            _env = env;
        ...
        public override void PreInitialize()
        {
            ...
            Configuration.DefaultNameOrConnectionString = _appConfiguration.GetConnectionString(
                CorarlERPConsts.ConnectionStringName
            );
            ...
            Configuration.Modules.AbpAspNetCore()
                .CreateControllersForAppServices(
                    typeof(CorarlERPApplicationModule).GetAssembly()
                );
            ...
            var redisConnection = _appConfiguration["Abp:RedisCache:ConnectionString"];
            if (!String.IsNullOrEmpty(redisConnection) && redisConnection != "localhost")
            {
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(CorarlERPWebCoreModule).GetAssembly());
        }

        public override void PostInitialize()
        {

            var defaultData = IocManager.Resolve<DefaultData>();

            InitDefaultPermissions(defaultData);

            SetAppFolders();

        }

        private void InitDefaultPermissions(DefaultData defaultData)
        {
            defaultData.StaticPermissions = new Dictionary<string, List<string>>();
            //VISAL: No need to assign permissions for Admin as we can grant all permissions to Admin
            defaultData.StaticPermissions = AppStaticRolePermissions.StaticPermissions;
        ...
        private void SetAppFolders()
        {
            ...
            appFolders.FontFolder = Path.Combine(_env.WebRootPath,"Common","Fonts");
            appFolders.TempFileDownloadFolder = Path.Combine(_env.WebRootPath, @"Temp/Downloads");
            appFolders.DownloadBaseUrl = "/file/DownloadTempFile";
            ...
            {
                var currentAssemblyDirectoryPath = typeof(CorarlERPWebCoreModule).GetAssembly().GetDirectoryPathOrNull();
