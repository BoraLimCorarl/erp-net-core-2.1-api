...
using CorarlERP.Authentication.TwoFactor.Google;
using CorarlERP.Authorization;
using CorarlERP.Authorization.Users;
using CorarlERP.MultiTenancy;
using CorarlERP.Web.Authentication.JwtBearer;
using CorarlERP.Web.Authentication.TwoFactor;
using CorarlERP.Web.Models.TokenAuth;
...
using CorarlERP.Web.Security.Recaptcha;
...
namespace CorarlERP.Web.Controllers
{
    ...
    public class TokenAuthController : CorarlERPControllerBase
    {
        ...
        public TokenAuthController(IApiClientManager apiClientManager,
        ...
        private async Task<ApplicationTypes> ValidateClientSecret(AuthenticationBaseModel model)
        {
        ...
        {
            return await AuthenticateHelper(model, GetTenancyNameOrNull());
        }

        [HttpPost]
        public async Task<AuthenticateResultModel> AuthenticateForMobile([FromBody] AuthenticateForMobileModel model)
        {
            return await AuthenticateHelper(model, model.Subdomain);
        }


        private async Task<AuthenticateResultModel> AuthenticateHelper(AuthenticateModel model, String subdomain)
        {
            var applicationType = await ValidateClientSecret(model);

            if (UseCaptchaOnLogin() && applicationType == ApplicationTypes.FrontEnd)
            {
            ...
            int? tenantId = loginResult.Tenant?.Id;
            //Password reset
        ...
        public async Task LogOut()
        {
            ...
            {
                var tokenValidityKeyInClaims = User.Claims.First(c => c.Type == CorarlERPConsts.TokenValidityKey);
        ...
        {
            return SimpleStringCipher.Instance.Encrypt(accessToken, CorarlERPConsts.DefaultPassPhrase);
        }
        ...
        private async Task<IEnumerable<Claim>> CreateJwtClaims(ClaimsIdentity identity, User user, TimeSpan? expiration = null)
        {
            ...
            claims.AddRange(new[]
            {
                ...
                new Claim(CorarlERPConsts.TokenValidityKey, tokenValidityKey),
                new Claim(CorarlERPConsts.UserIdentifier, userIdentifier.ToUserIdentifierString())
            });
        ...
        [HttpPost]
        public async Task<GetTokenOutput> RefreshToken([FromBody] AuthenticateModel model)
        {
            ...
            UserIdentifier userIdentifier = new UserIdentifier(null, loginResult.User.Id);
           // var isgrented = await PermissionChecker.IsGrantedAsync(AppPermissions.Pages_Host_RefreshToken);
            var isgrented = await PermissionChecker.IsGrantedAsync(userIdentifier, AppPermissions.Pages_Host_RefreshToken);
            if (!isgrented) throw new Exception(L("NoPermission"));
            // var returnUrl = model.ReturnUrl;
            ...
            //Login!
            var expiration = int.Parse(_configurationAccessor.Configuration["Authentication:ExpirationDays"]);
            var accessToken = CreateAccessToken(await CreateJwtClaims(loginResult.Identity, loginResult.User, TimeSpan.FromDays(expiration)));
            var exp = DateTime.UtcNow.AddDays(expiration).AddHours(-3);           
            return new GetTokenOutput { Expired = exp, Token = accessToken };
        }
