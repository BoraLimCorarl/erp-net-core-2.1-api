...
using CorarlERP.Auditing.Exporting;
using CorarlERP.Authorization;
using CorarlERP.Authorization.Users;
using CorarlERP.Dto;
using EntityHistoryHelper = CorarlERP.EntityHistory.EntityHistoryHelper;
...
namespace CorarlERP.Auditing
{
    ...
    public class AuditLogAppService : CorarlERPAppServiceBase, IAuditLogAppService
    {
        ...
        {
            var resultCount = 0;
            var results = new List<AuditLogAndUser>();
        ...
        {
            var auditLogs = new List<AuditLogAndUser>();

            using (var uow = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {
                using (_unitOfWorkManager.Current.SetTenantId(input.TenantId))
                {
                    auditLogs = await CreateAuditLogAndUsersQuery(input)
                    .AsNoTracking()
        ...
        {
            var resultCount = 0;
            var results = new List<EntityChangeAndUser>();
        ...
        {
            var entityChanges = new List<EntityChangeAndUser>();

            using (var uow = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {
                using (_unitOfWorkManager.Current.SetTenantId(input.TenantId))
                {
                    entityChanges = await CreateEntityChangesAndUsersQuery(input)
                        .AsNoTracking()
        ...
        [UnitOfWork(IsDisabled = true)]
        public async Task<List<EntityPropertyChangeDto>> GetEntityPropertyChanges(int? tenantId, long entityChangeId)
        {
            IEnumerable<EntityPropertyChange> entityPropertyChanges;

            using (var uow = _unitOfWorkManager.Begin(TransactionScopeOption.RequiresNew))
            {
                using (_unitOfWorkManager.Current.SetTenantId(tenantId))
                {
                    entityPropertyChanges = (await _entityPropertyChangeRepository.GetAllListAsync())
