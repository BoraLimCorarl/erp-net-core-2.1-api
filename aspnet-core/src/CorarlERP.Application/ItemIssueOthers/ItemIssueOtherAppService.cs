...
{
    [AbpAuthorize]
    public class ItemIssueOtherAppService : CorarlERPAppServiceBase, IItemIssueOtherAppService
    {
        ...
        public ItemIssueOtherAppService(
        ...
        IAutoSequenceManager autoSequenceManager): base(accountCycleRepository,null,null)
        {
            ...
            _journalManager.SetJournalType(JournalType.ItemIssueOther);
        ...
        private async Task ValidateBatchNo(CreateItemIssueOtherInput input)
        {
            ...
            if (input is UpdateItemIssueOtherInput)
            {
                exceptIds.Add((input as UpdateItemIssueOtherInput).Id);
            }
        ...
        [AbpAuthorize(AppPermissions.Pages_Tenant_Customers_ItemIssues_CanCreateOther,
                      AppPermissions.Pages_Tenant_Inventorys_InventoryTransactions_CreateItemIssueOther)]
        public async Task<NullableIdDto<Guid>> Create(CreateItemIssueOtherInput input)
        {
            ...
            itemIssue.UpdateTransactionType(InventoryTransactionType.ItemIssueOther);
            @entity.UpdateIssueOther(itemIssue);
        ...
        public async Task<ItemIssueOtherDetailOutput> GetDetail(EntityDto<Guid> input)
        {
            ...
            var @journal = await _journalRepository
                              ...
                              .AsNoTracking()
                              .Where(u => u.JournalType == JournalType.ItemIssueOther && u.ItemIssueId == input.Id)
                              .FirstOrDefaultAsync();
            ...
            var itemIssueItems = await (from rItem in _itemIssueItemRepository.GetAll()
                                        ...
                                        select
                                        new ItemIssueItemOtherDetailOutput()
                                        {
                                            LotId = rItem.LotId,
            ...
            var result = ObjectMapper.Map<ItemIssueOtherDetailOutput>(journal.ItemIssue);
            ...
            result.ItemIssueItemOthers = itemIssueItems;
        ...
        public async Task<NullableIdDto<Guid>> Update(UpdateItemIssueOtherInput input)
        {
            if (input.InventoryTransactionTypeId == null || input.InventoryTransactionTypeId == Guid.Empty) throw new UserFriendlyException(L("TransactionTypeIdNotFound"));
            var tenantId = AbpSession.GetTenantId();
            var userId = AbpSession.GetUserId();
