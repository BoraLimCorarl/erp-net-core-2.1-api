...
{
    [AbpAuthorize]
    public class ProfileAppService : CorarlERPAppServiceBase, IProfileAppService
    {
        ...
        public async Task<CurrentUserProfileEditDto> GetCurrentUserProfileForEdit()
        {
            ...
            userProfileEditDto.QrCodeSetupImageUrl = user.GoogleAuthenticatorKey != null
                ? _googleTwoFactorAuthenticateService.GenerateSetupCode("CorarlERP",
        ...
        public async Task<UpdateGoogleAuthenticatorKeyOutput> UpdateGoogleAuthenticatorKey()
        {
            ...
            {
                QrCodeSetupImageUrl = _googleTwoFactorAuthenticateService.GenerateSetupCode("CorarlERP",
                    user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300).QrCodeSetupImageUrl
            };
        }
        ...
        public async Task UpdateProfilePicture(UpdateProfilePictureInput input)
        {
            ...
            {
                throw new UserFriendlyException(L("ResizedProfilePicture_Warn_SizeLimit", CorarlERPConsts.ResizedMaxProfilPictureBytesUserFriendlyValue));
            }
        ...
        public async Task<UpdateUserProfileImageInput> GetUserProfileImageId()
        {
            var user = await UserManager.GetUserByIdAsync(AbpSession.GetUserId());
            return new UpdateUserProfileImageInput { UserImgId = user.ProfilePictureId };
        }
        public async Task UpdateProfileImage(UpdateUserProfileImageInput input)
        {

            var user = await UserManager.GetUserByIdAsync(AbpSession.GetUserId());
            user.ProfilePictureId = input.UserImgId;
            await UserManager.UpdateAsync(user);
