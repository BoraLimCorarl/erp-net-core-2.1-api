...
using CorarlERP.Authorization.Roles.Dto;
using Abp.UI;
...
namespace CorarlERP.Authorization.Roles
{
    ...
    public class RoleAppService : CorarlERPAppServiceBase, IRoleAppService
    {
        ...
        [AbpAuthorize(AppPermissions.Pages_Administration_Roles_GetList)]
        ...
        protected virtual async Task UpdateRoleAsync(CreateOrUpdateRoleInput input)
        {
            ...
            if (tenantId != null && role.IsStatic && role.IsDefault == input.Role.IsDefault)
            {
                throw new UserFriendlyException(L("CanNotUpdateRoleStatic"));
            }      
