...
namespace CorarlERP.Editions
{
    ...
    public class EditionAppService : CorarlERPAppServiceBase, IEditionAppService
    {
        ...
        public EditionAppService(
            ...
            IRepository<SubscribableEdition> subscribableEditionRepository, IRepository<Edition, int> editionRepository)
        {
        ...
        public async Task<List<SubscribableEditionComboboxItemDto>> GetEditionComboboxItems(int? selectedEditionId = null, bool addAllItem = false, bool onlyFreeItems = false)
        {
            ...
            var editionItems =
                new ListResultDto<SubscribableEditionComboboxItemDto>(subscribableEditions
                    .Select(e => new SubscribableEditionComboboxItemDto(e.Id.ToString(), e.DisplayName, e.IsFree, e.TrialDayCount, e.DailyPrice, e.IsPaid)).ToList()).Items.ToList();

            var defaultItem = new SubscribableEditionComboboxItemDto("", L("NotAssigned"), null, null, null, false);
            editionItems.Insert(0, defaultItem);

            if (addAllItem)
            {
                editionItems.Insert(0, new SubscribableEditionComboboxItemDto("-1", "- " + L("All") + " -", null, null, null, false));
            }
        ...
        private async Task CheckDuplicateEdition(Edition @entity)
        {
            var @old = await _editionRepository.GetAll().AsNoTracking()
            ...
            if (old != null)
            {
                throw new UserFriendlyException(L("DuplicateEditionName", entity.Name));
            }
        }
        ...
        protected virtual async Task CreateEditionAsync(CreateOrUpdateEditionDto input)
        {
            ...
            edition.Name = input.Edition.Name;
            await this.CheckDuplicateEdition(edition);
