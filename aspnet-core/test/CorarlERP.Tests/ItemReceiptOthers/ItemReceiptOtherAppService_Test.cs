...
using CorarlERP.ItemReceiptOthers;
...
namespace CorarlERP.Tests.ItemReceiptOthers
{
  public  class ItemReceiptOtherAppService_Test : AppTestBase
    {
        private readonly IItemReceiptOtherAppService _itemReceiptOtherAppService;
        ...
        public ItemReceiptOtherAppService_Test()
        {
            _itemReceiptOtherAppService = Resolve<IItemReceiptOtherAppService>();
        ...
        private CreateItemReceiptOtherInput CreateHelper()
        {
            var ItemReceiptExemptInput = new CreateItemReceiptOtherInput()
            {
                ...
                Items = new List<CreateOrUpdateItemReceiptItemOtherInput>()
                {
                    new CreateOrUpdateItemReceiptItemOtherInput(){InventoryAccountId=_chartOfAccount.Id,DiscountRate=10,
        ...
        private void CheckItemReceipt(CreateItemReceiptOtherInput JournalExemptInput, Guid? Id = null)
        {
        ...
        public async Task Test_CreateItemReceipt()
        {
            ...
            var result = await _itemReceiptOtherAppService.Create(ItemReceiptExemptInput);
        ...
        public async Task Test_Update()
        {
            ...
            var UpdateItemReceiptExemptInput = new UpdateItemReiptOtherInput()
            {
                Id = result.Id.Value,
                CurrencyId = _currency.Id,
                ...
                Items = new List<CreateOrUpdateItemReceiptItemOtherInput>()
                {
                    new CreateOrUpdateItemReceiptItemOtherInput(){InventoryAccountId=_chartOfAccount.Id,DiscountRate=10,
                     ...
                     new CreateOrUpdateItemReceiptItemOtherInput(){InventoryAccountId = _chartOfAccount.Id,DiscountRate = 100,
                        Description ="Order From Thai",ItemId = _item.Id, Qty = 10,Total=100,UnitCost = 100,
                    },
                }

            };
            var updatedResult = await _itemReceiptOtherAppService.Update(UpdateItemReceiptExemptInput);
        ...
        public async Task Test_GetDetail()
        {
            ...
            var result = await _itemReceiptOtherAppService.Create(JournalExemptInput);

            result.Id.ShouldNotBeNull();
            var output = await _itemReceiptOtherAppService.GetDetail(new EntityDto<Guid>()
            {
